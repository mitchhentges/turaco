...
public class QMReducer {
  ...
  public void setBoolTT(boolean[][] aTT) {
    ...
    if (isConst) {
      this.isConst = true;
      this.constVal = (prevVal == 1 ? true : false);
    }
  }
  ...
  public String reduce(ArrayList varNames) {
      ...
      for (int i = 0; i < this.groups.length; i++) {
        for (int s = 0; s < this.groups[i].size(); s++) {
          if (!this.groups[i].get(s).isUsed())
            unreducableGroup.add(this.groups[i].get(s));
        }
        this.groups[i].clear();
      }
    ...
    QMGroup reducedExpr = new CoverTable(unreducableGroup).reduce();
    ...
    int toPrint;
    for (int i = 0; i < reducedExpr.size(); i++) {
      for (int c = 0; c < this.groups.length - 1; c++) {
        toPrint = reducedExpr.get(i).getRow()[c];
        if (toPrint == -1)
          System.out.print("x");
        else
  ...
  public static void main(String[] args) {
    QMGroup aGroup = new QMGroup();
    System.out.println(aGroup.equals(aGroup.clone()));
  }
}
...
class QMGroup {
  ...
  public void addAll(QMGroup aGroup) {
    for (int i = 0; i < aGroup.size(); i++) {
      this.add(aGroup.get(i));
    }
  }
  ...
  public boolean equals(Object anObj) {
    if (anObj == null || !(anObj instanceof QMGroup))
      return false;

    return ((QMGroup) anObj).aList.equals(this.aList);
  }
...
class QMItem {
  ...
  public boolean equals(Object o) {
    if (o == null || !(o instanceof QMItem))
      return false;

    return Arrays.equals(this.row, ((QMItem) o).row);
  }
